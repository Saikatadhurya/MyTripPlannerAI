...
const App: React.FC = () => {
  ...
  const handleStartPlanning = useCallback((destination?: string) => {
    ...
    setFormData(destination ? { destination, startPoint: '', tripType: 'Standard', days: 3, budget: 'Midrange', vibe: defaultVibes, persons: 1, foodPreference: 'Non-Veg', startDate: today, includeMedical: false, language: 'English (en)', isRoundTrip: false } : null);
  ...
  const handleGenerateItinerary = useCallback(async (data: QuestionnaireData) => {
    ...
    setFormData(data);
    try {
      const generatedItinerary = await generateItinerary(data.destination, data.startPoint, data.tripType, data.days, data.budget, data.vibe, data.persons, data.foodPreference, data.startDate, data.includeMedical, data.language, data.isRoundTrip);
