...
export const generateItinerary = async (
  ...
  **CORE ITINERARY PHILOSOPHY: MAXIMALIST & EFFICIENT**
  ...
  11. For 'historicBackground', 'famousCulture', 'naturalPlaces', 'museums', 'specialOrnaments', and 'recommendedRestaurants', provide a list of 1-3 very concise bullet points. Each point must be a short, descriptive phrase (around 5-10 words maximum). For restaurants, this can be just the name. Do not provide a single paragraph.
  ...
  13. **Currency Conversion (CRITICAL)**:
      ...
      c. If they are different, you MUST populate the 'currencyConversion' object in the JSON response. Provide a simple, clear text representation of the approximate exchange rate in the 'rateText' field. The format MUST be '[VALUE] [DESTINATION CURRENCY CODE] ≈ 1 [SOURCE CURRENCY CODE]'. For example, if the destination is India (INR) and the user's currency is USD, the text should be like '83 INR ≈ 1 USD'. The 'fromCurrency' MUST be the user's chosen currency code (e.g., 'USD'), and 'toCurrency' MUST be the destination's local currency code (e.g., 'INR').
